...
fuzz_target!(|data: &[u8]| {
    ...
    if data.len() >= 2 {
        ...
        if let Some(entry) = fjell_semantic_v1::lookup_tag(tag) {
            let mut fields =
                [fjell_semantic_v1::FieldValue::Absent; fjell_semantic_v1::codec::MAX_FIELDS];
            for (i, fd) in entry.schema.fields.iter().enumerate() {
                use fjell_semantic_v1::schema::FieldKind;
                fields[i] = match fd.kind {
                    FieldKind::U8 => {
                        fjell_semantic_v1::FieldValue::U8(data.get(2).copied().unwrap_or(0))
                    }
            ...
            if let Ok(n) =
                fjell_semantic_v1::encode(tag, 0, &fields[..entry.schema.fields.len()], &mut buf)
            {
