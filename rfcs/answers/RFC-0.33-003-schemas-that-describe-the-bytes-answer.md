...
| **`attestation v2`** | the file lists **16** fields; the stream has **~50** parts, including whole groups the file does not mention — keyring, boot, verification, snapshot, health, rollback, freshness, provenance. |
...
## R1 addendum — the drift, field by field, for all eleven (measured by the instrument, **before** any file was regenerated)

Produced by `cargo xtask schema check` (each committed file against what its
encoder generates), run at the tip where every encoder already writes through
`Canon` and every golden digest holds — so "the encoder" here is exactly what the
code did before this line touched it. The header line each file opens with
(`# generated by fjell-tools schema dump`, naming a subcommand that did not exist)
differs in all eleven and is left out of the counts. `file-only` = a field the
committed file claims that the encoder does not write (or writes under another
name/width); `encoder-only` = a field the encoder writes that the file does not
mention. A field renamed or re-widthed counts once on each side; the drift lines
name both.

| File | file-only | encoder-only | What is wrong |
|---|---|---|---|
| `rollback-record-v1` | 2 | 4 (+1 width) | `channel u8[16]` is `channel_id u8[8]`; `min_counter` `u32` is `u64`; `updated_tick` is `last_advance_tick`; **absent from the file**: `last_advance_source u8` and the 32-byte `record_digest_placeholder`. |
| `release-metadata-v1` | 11 | 12 | **A different structure**, not a stale one: the file's domain is `FJELL-RELEASE-V1`, the encoder's `FJELL-RELEASE-META-V1`; none of `release_id`, `channel`, `counter`, `min_counter`, the three digests or the `signature.*` triple is written; twelve parts the file never mentions are. |
| `attestation v2` | 10 | 35 | The file lists 16 fields; the encoder writes the groups `keyring`, `boot`, `verification`, `measurement`, `snapshot`, `health`, `rollback`, `freshness` and `provenance` that the file has no word for, and `provider.kind/gen` are `provider.provider_kind/provider_generation`. |
| `keyring snapshot-v1` | 4 | 10 (+ a `group`) | The file describes `purpose_count` and six `purpose[0..6]` records with a 64-byte `anchor_bytes`; the stream is the `SNAP-V1` domain, `anchor_count` and **28** slots of `{present, purpose, algorithm, authority, epoch, reserved, key_len, key_bytes[key_len]}`. |
| `diag bundle-v1` | 7 | 13 (+ 2 `group`s) | The file describes `export_tick`, `source_service`, `entry_count` and `entries[].{tag,tick,len,body}`; the stream is `created_tick`, `measurement_head`, `last_attestation` and two counted groups (`audit_events` ≤ 64, `semantic_intents` ≤ 32), each of `{seq, tag, code, at_tick}`. |
| `board-v1` | 7 | 7 (+ a `group`) | **Spelling only**: `devices[0..15].x` is the same seven fields as `devices[].x` under `group devices max 16`. No field differs. |
| `intent-v1` | 5 | 7 (+ 4 notes) | The file's single `fields[].value u8 [FieldKind.wire_size]` is a shape no encoder call produces; and its hand-typed `catalog_entries 20` / `catalog_version` lines have no source (both are now derived from the catalogue). |
| `node-identity-v1` | 0 | 0 | **Agrees exactly.** |
| `measurement-summary-v1` | 0 | 0 (+ a `group`) | Agrees; the group's capacity is now stated. |
| `release-summary-v1` | 0 | 0 (+ a `group`) | Agrees; same. |
| `snapshot-v2` | 0 | 0 (+ a `group`) | Agrees; same. The conditional `domain` byte (from `schema_version` 2) is described for the version the file is *for*. |

So **three of eleven agree, one differs by spelling, and seven describe something
other than what is hashed** — worst two (`release-metadata`, `attestation v2`) not
even the same shape. The RFC's one example (`rollback-record`) was the mildest
kind of the seven, not a typical one.

**Four encoders have no file at all**, found by the same registry (§C): the
platform profile's digest stream (`platform-v1`; `board-v1` has a file, its sibling
does not) and the three disk structures. Their files are generated fresh — there is
nothing to have drifted from, which is itself the finding.

**What the description does not carry** (stated so it is not mistaken for more): it
carries **names, types, widths, order, counted groups with their capacity, and
constants where the encoder marks one** (`domain`, `magic`, a sentinel via
`Canon::constant`). It does not carry *values* of ordinary fields (an enum's
discriminant meaning, what a `present` byte's `+`/`-` stand for) — those live in
the encoder and its tests, and the golden digests hold them.
