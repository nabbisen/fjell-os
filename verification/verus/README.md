...
Machine-checked: **20 obligations verified, 0 errors** (v0.18.1) under verus
...
Each proof maps 1:1 to the shipped predicate.

Current gate levels (v0.20.0): `capability` and `lease` are
`release_required = true` (promoted v0.18.0, RFC-v0.18-001); `boot-control`
is `release_required = false` (tier 2 / pilot-required; promotion scheduled
after the next development cycle per architect D5).
