fn main() {
    ...
    let key_path = std::path::Path::new(&manifest)
        .join("../../provision/dev-trust-anchor.key");
    ...
    let (key_bytes, provisioned): ([u8; 32], bool) = match std::fs::read_to_string(&key_path) {
        Ok(hex) => {
            let hex = hex.trim();
            let mut k = [0u8; 32];
            let ok = hex.len() == 64 && (0..32).all(|i| {
                u8::from_str_radix(&hex[i*2..i*2+2], 16)
                    .map(|b| { k[i] = b; true }).unwrap_or(false)
            });
            if !ok {
                panic!("provision/dev-trust-anchor.key exists but is not 64 hex chars — \
