...
pub mod renderer;
pub use renderer::{
    BannerInfo, DEFAULT_WIDTH, MAX_PINNED, ProxyState, RateLimitEntry, RenderedEntry,
    SCROLL_BUFFER, ScrollRing, is_critical_tag,
};
...
fn w(s: &str) {
    fjell_syscall::sys_debug_write(s);
}
fn wln(s: &str) {
    fjell_syscall::sys_debug_writeln(s);
}
...
pub fn render_event(n: &EventNode) {
    ...
    if !n.description.fallback.is_empty() {
        wln("Description:");
        w("  ");
        wln(n.description.as_str());
    }
    if let Some(seq) = n.related_audit_seq {
        let mut buf = [0u8; 20];
        wln("Audit:");
        w("  seq = ");
        wln(u64_str(seq, &mut buf));
    }
    wln("");
}
