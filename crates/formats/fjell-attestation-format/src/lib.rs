...
pub mod v2;
pub use v2::{
    ATTEST_V2_DOMAIN, AttestationRecordV2, FreshnessClaimsV2, KeyringClaims, NonceClass,
...
impl ToStringRadix for u32 {
    fn to_string_radix(&self, _: u32) -> U32Str {
        ...
        if n == 0 {
            buf[0] = b'0';
            return U32Str { buf, len: 1 };
        }
        while n > 0 {
            buf[9 - len] = b'0' + (n % 10) as u8;
            n /= 10;
            len += 1;
        }
        let mut out = [0u8; 10];
...
impl AttestationRecord {
    ...
    pub fn canonical_digest(&self) -> Digest32 {
        ...
        let rr = [
            self.verification.release_verified as u8,
