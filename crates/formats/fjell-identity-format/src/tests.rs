...
use crate::identity::{
    AttestationPubkey, IdentityError, NODE_IDENTITY_SCHEMA_VERSION, NodeAlias, NodeId,
    NodeIdentity, NodeIdentityBuilder, STORE_RECORD_KIND_IDENTITY,
};
use crate::policy::{Decision, NodeIdentityPolicy, PolicyError, RosterRef, TrustMode};
...
fn fleet_policy_with_roster_needs_validation() {
    ...
    assert!(matches!(
        p.permits(0x01),
        Decision::NeedsRosterValidation(_)
    ));
}
