...
impl AuditRecordBin {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Option<Self> {
        if bytes.len() < AUDIT_RECORD_BIN_SIZE {
            return None;
        }
        Some(Self {
...
const _: () = assert!(core::mem::size_of::<AuditPersistRecord>() == AUDIT_PERSIST_RECORD_SIZE);
...
const _: () = assert!(core::mem::size_of::<AuditLogHeader>() == AUDIT_RECORD_BIN_SIZE);
...
mod tests {
    ...
    fn audit_kind_v2_labels() {
        ...
        assert_eq!(
            AuditKind::TaskQuantumExceeded.label(),
            "task.quantum_exceeded"
        );
    }
