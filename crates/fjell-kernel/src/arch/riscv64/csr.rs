...
#[inline]
// SAFETY: category=kernel-global-mutable CSR access is valid in S-mode; register name is correct for riscv64gc.
pub unsafe fn read_mhartid() -> usize {
    let v: usize;
    // SAFETY: category=csr-asm read-only CSR, no side effects.
...
pub unsafe fn read_sstatus() -> usize {
    let v: usize;
    // SAFETY: category=csr-asm read-only CSR access.
...
pub unsafe fn write_sstatus(v: usize) {
    // SAFETY: category=csr-asm caller upholds CSR write invariants.
...
pub unsafe fn write_sepc(v: usize) {
    // SAFETY: category=csr-asm caller guarantees `v` is a valid user-space address.
...
pub unsafe fn write_stvec(addr: usize) {
    // SAFETY: category=csr-asm caller ensures `addr` points to a valid trap handler with
...
pub unsafe fn write_sscratch(v: usize) {
    // SAFETY: category=csr-asm caller owns the sscratch convention.
...
pub unsafe fn write_sie(v: usize) {
    // SAFETY: category=csr-asm caller is responsible for interrupt-enable timing.
...
pub unsafe fn write_mstatus(v: usize) {
    // SAFETY: category=csr-asm M-mode shim context; caller upholds M-mode invariants.
...
pub unsafe fn write_medeleg(v: usize) {
    // SAFETY: category=csr-asm caller selects only safe-to-delegate exception bits.
...
pub unsafe fn write_mideleg(v: usize) {
    // SAFETY: category=csr-asm caller selects only safe-to-delegate interrupt bits.
...
pub unsafe fn write_mepc(v: usize) {
    // SAFETY: category=csr-asm caller provides the valid S-mode entry address.
...
// SAFETY: category=csr-asm CSR access is valid in S-mode; register name is correct for riscv64gc.
pub unsafe fn mret() -> ! {
    // SAFETY: category=csr-asm caller has correctly set mepc (S-mode entry) and mstatus.MPP = S.
    unsafe {
