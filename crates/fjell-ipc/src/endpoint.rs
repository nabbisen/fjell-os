...
use super::message::{IPC_WORDS, MessageTag};
...
impl RecvWaiter {
    ...
    pub fn no_lease(tid: u16) -> Self {
        RecvWaiter { tid, lease: None }
    }
...
impl CancelledByLease {
    fn empty() -> Self {
        CancelledByLease {
            sender_tids: [0; QUEUE_DEPTH],
            receiver_tids: [0; QUEUE_DEPTH],
            n_senders: 0,
            n_receivers: 0,
        }
    }
    /// Iterate over cancelled sender TIDs.
    pub fn senders(&self) -> &[u16] {
        &self.sender_tids[..self.n_senders]
    }
    /// Iterate over cancelled receiver TIDs.
    pub fn receivers(&self) -> &[u16] {
        &self.receiver_tids[..self.n_receivers]
    }
}
...
impl From<EndpointError> for SysError {
    fn from(e: EndpointError) -> Self {
        match e {
            EndpointError::SendQueueFull | EndpointError::RecvQueueFull => SysError::QueueFull,
...
impl Endpoint {
    pub const fn new() -> Self {
        Endpoint {
            sendq: FixedQueue::new(),
            recvq: FixedQueue::new(),
        }
    }

    pub fn send(&mut self, msg: PendingMessage) -> Result<SendResult, EndpointError> {
        if !msg.tag.is_valid() {
            return Err(EndpointError::InvalidTag);
        }
        ...
        if let Some(w) = self.recvq.pop() {
            return Ok(SendResult::Delivered {
                receiver_tid: w.tid,
            });
        }
        if !self.sendq.push(msg) {
            return Err(EndpointError::SendQueueFull);
        }
    ...
    pub fn recv(&mut self, waiter: RecvWaiter) -> Result<RecvResult, EndpointError> {
        if self
            .sendq
            .contains(|m: PendingMessage| m.sender_tid == waiter.tid)
        {
            return Err(EndpointError::AlreadyQueued);
        }
        ...
        if !self.recvq.push(waiter) {
            return Err(EndpointError::RecvQueueFull);
        }
    ...
    pub fn cancel(&mut self, tid: u16) {
        let mut rm = [PendingMessage {
            tag: MessageTag::new(0, 0, 0),
            sender_tid: 0,
            sender_image_id: 0,
            sender_badge: 0,
            words: [0; IPC_WORDS],
        ...
        let mut n = 0;
        self.sendq
            .retain_collect(|m: PendingMessage| m.sender_tid != tid, &mut rm, &mut n);

        let mut rw = [RecvWaiter {
            tid: 0,
            lease: None,
        }; QUEUE_DEPTH];
        n = 0;
        self.recvq
            .retain_collect(|w: RecvWaiter| w.tid != tid, &mut rw, &mut n);
    }
    ...
    pub fn cancel_by_lease(&mut self, lease_id: LeaseId, epoch: u32) -> CancelledByLease {
...
mod tests {
    ...
    fn lb(idx: u16, epoch: u32) -> LeaseBinding {
        LeaseBinding {
            lease_id: LeaseId::new(idx, 1),
            epoch_at_issue: LeaseEpoch(epoch),
        }
    }
    ...
    fn send_to_waiting_receiver() {
        ...
        assert!(matches!(
            ep.recv(RecvWaiter::no_lease(10)).unwrap(),
            RecvResult::Queued
        ));
    ...
    fn recv_from_waiting_sender() {
        let mut ep = Endpoint::new();
        assert!(matches!(
            ep.send(msg(5, 99, true, None)).unwrap(),
            SendResult::Queued
        ));
    ...
    fn queue_full_error() {
        let mut ep = Endpoint::new();
        for i in 0..32u16 {
            ep.send(msg(i, 0, false, None)).unwrap();
        }
        assert_eq!(
            ep.send(msg(99, 0, false, None)).unwrap_err(),
