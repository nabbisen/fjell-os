...
pub fn parse_header(dtb: &[u8]) -> Result<DtbHeader, ParseError> {
    ...
    if (off_dt_struct as usize) >= dtb.len() {
        return Err(ParseError::BadStructOffset);
    }
    if (off_dt_strings as usize) >= dtb.len() {
        return Err(ParseError::BadStrOffset);
    }
    Ok(DtbHeader {
        totalsize,
        off_dt_struct,
        ...
        loop {
            ...
            match tok {
                ...
                FDT_PROP => {
                    ...
                    let val_size = be32(self.dtb, self.pos)? as usize;
                    self.pos += 4;
                    let name_off = be32(self.dtb, self.pos)?;
                    self.pos += 4;
                    let val_start = self.pos;
                    if val_start + val_size > self.end {
                        return Some(Err(ParseError::Truncated));
                    }
                    let value = &self.dtb[val_start..val_start + val_size];
                    self.pos += val_size;
                    self.pos = (self.pos + 3) & !3;
                    let name = get_string(self.dtb, self.str_off, name_off).unwrap_or(b"");
