...
use crate::compat::{ALLOWED_COMPAT_QEMU_VIRT, CompatString};
use crate::parser::{FdtIter, FdtProp, NodeEvent, ParseError, parse_header};
use fjell_measure_format::Digest32;
use fjell_platform_format::{
    BoardDevice, BoardProfile, DeviceClass, PlatformProfile, RecoveryDescriptor, RecoveryKind,
    board_digest, platform_digest,
};
...
pub fn derive_board_profile(
        ...
        match event.map_err(DeriveError::DtbParseFailed)? {
            ...
            NodeEvent::EndNode => {
                if node_depth == 2 {
                    if let Some(class) = cur_class {
                        if count >= 16 {
                            return Err(DeriveError::TooManyDevices);
                        }
            ...
            NodeEvent::Prop(FdtProp { name: prop, value }) => {
                ...
                if prop == b"compatible" {
                    ...
                    cur_irq = u32::from_be_bytes([value[0], value[1], value[2], value[3]]) as u16;
                }
            }
        }
    }
    ...
    if !found_plic {
        return Err(DeriveError::MissingPlic);
    }
...
fn classify_compat(value: &[u8]) -> Option<DeviceClass> {
    for part in value.split(|&b| b == 0) {
        ...
        if part == b"ns16550a" {
            return Some(DeviceClass::Uart8250);
        }
        if part == b"virtio,mmio" {
            return Some(DeviceClass::VirtioNetMmio);
        }
        if part == b"riscv,plic0" {
            return Some(DeviceClass::Plic);
        }
        if part == b"riscv,clint0" {
            return Some(DeviceClass::Clint);
        }
    }
    None
}
