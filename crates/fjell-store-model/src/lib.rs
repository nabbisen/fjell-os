...
pub fn s1_replay_idempotent(model: &mut LogModel) -> Result<(), String> {
    ...
    if first != second {
        return Err(format!(
            "S1: replay not idempotent: first={} second={}",
            first.len(),
            second.len()
        ));
    }
...
pub fn s2_latest_authoritative(model: &mut LogModel) -> Result<(), String> {
    ...
    for kind in [RecordKind::Policy, RecordKind::Snapshot, RecordKind::Health] {
        ...
        match (expected, got) {
            ...
            (Some(e), Some(g)) => {
                return Err(format!(
                    "S2: latest({kind:?}) expected seq={} got seq={}",
                    e.seq, g.seq
                ));
            }
            (Some(e), None) => {
                return Err(format!(
                    "S2: latest({kind:?}) expected seq={} got None",
                    e.seq
                ));
            }
            (None, Some(g)) => {
                return Err(format!(
                    "S2: latest({kind:?}) expected None got seq={}",
                    g.seq
                ));
            }
        }
...
pub fn s3_crash_drops_partial(model: &mut LogModel, crash_tail: usize) -> Result<(), String> {
    ...
    if model.replayed.len() > kept {
        return Err(format!(
            "S3: expected ≤{kept} records after crash, got {}",
            model.replayed.len()
        ));
    }
    Ok(())
...
pub fn s4_corrupt_record_skipped(model: &mut LogModel) -> Result<(), String> {
    ...
    if has_corrupt {
        return Err("S4: corrupt record survived restart".into());
    }
    if !has_good {
        return Err("S4: valid record lost after corrupt neighbour".into());
    }
    Ok(())
}
...
pub fn s5_rollback_replay_highest(model: &mut LogModel) -> Result<(), String> {
    ...
    if seqs.len() >= 2 {
        ...
        if let Some(l) = model.latest(RecordKind::Rollback) {
            if l.seq != max_seq {
                return Err(format!(
                    "S5: latest Rollback seq={} not max={max_seq}",
                    l.seq
                ));
            }
        }
...
pub fn s6_staging_replay_coherent(model: &mut LogModel) -> Result<(), String> {
    ...
    let staging_seqs: Vec<u64> = model
        .replayed
        .iter()
...
fn arb_record_kind() -> impl Strategy<Value = RecordKind> {
    prop_oneof![
        Just(RecordKind::Boot),
        Just(RecordKind::Policy),
        Just(RecordKind::Snapshot),
        Just(RecordKind::Staging),
        Just(RecordKind::Rollback),
        Just(RecordKind::Health),
    ]
}
