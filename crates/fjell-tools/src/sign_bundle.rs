...
pub fn cmd_sign_bundle(args: &[String]) -> ExitCode {
    ...
    let (bundle_path, key_path, out_path) = match (bundle_path, key_path, out_path) {
        ...
        _ => {
            eprintln!(
                "Usage: cargo xtask sign-bundle --bundle <f> --key <k> --out <sig> [--key-id <hex>]"
            );
    ...
    let bundle_bytes = match fs::read(&bundle_path) {
        Ok(b) => b,
        Err(e) => {
            eprintln!("sign-bundle: cannot read bundle: {}", e);
            return ExitCode::FAILURE;
        }
    };
    ...
    match fs::write(&out_path, &manifest) {
        ...
        Err(e) => {
            eprintln!("sign-bundle: write error: {}", e);
            ExitCode::FAILURE
        }
    }
}
...
pub fn cmd_verify_bundle_sig(args: &[String]) -> ExitCode {
    ...
    let (bundle_path, sig_path, pubkey_hex) = match (bundle_path, sig_path, pubkey_hex) {
        ...
        _ => {
            eprintln!(
                "Usage: cargo xtask verify-bundle-sig --bundle <f> --sig <s> --pubkey <hex32>"
            );
            return ExitCode::FAILURE;
        }
    };
    ...
    let pubkey: [u8; 32] = match unhex32(&pubkey_hex) {
        Ok(b) => b,
        Err(e) => {
            eprintln!("verify-bundle-sig: --pubkey: {}", e);
            return ExitCode::FAILURE;
        }
    };
    ...
    match verify_sigmanifest(&bundle_bytes, &sig_bytes, &pubkey) {
        Ok(()) => {
            println!("verify-bundle-sig: PASS");
            ExitCode::SUCCESS
        }
        Err(e) => {
            eprintln!("verify-bundle-sig: FAIL — {}", e);
            ExitCode::FAILURE
        }
    }
}
...
fn key_gen(args: &[String]) -> ExitCode {
    let out_path = match flag(args, "--out") {
        Some(p) => p,
        None => {
            eprintln!("key gen: --out <file> required");
            return ExitCode::FAILURE;
        }
    };
    ...
    match getrandom::getrandom(&mut seed) {
        Ok(_) => {}
        Err(e) => {
            eprintln!("key gen: getrandom failed: {}", e);
            return ExitCode::FAILURE;
        }
    };
    ...
    let key_file: Vec<u8> = if insecure {
        ...
        let pass = match crate::key_crypto::resolve_passphrase(args) {
            ...
            None => {
                eprintln!(
                    "key gen: {}",
                    crate::key_crypto::KeyCryptoError::NoPassphrase
                );
    ...
    match fs::write(&out_path, &key_file) {
        ...
        Err(e) => {
            eprintln!("key gen: write error: {}", e);
            ExitCode::FAILURE
        }
    }
}
...
fn key_show(args: &[String]) -> ExitCode {
    let in_path = match flag(args, "--in") {
        Some(p) => p,
        None => {
            eprintln!("key show: --in <file> required");
            return ExitCode::FAILURE;
        }
    };
...
fn read_pubkey_only(path: &str) -> Result<[u8; 32], String> {
    ...
    if crate::key_crypto::is_encrypted(&bytes) {
        if bytes.len() < 66 {
            return Err("truncated FJK2 key file".into());
        }
...
fn load_raw_key_with_args(path: &str, args: &[String]) -> Result<([u8; 32], [u8; 32]), String> {
    ...
    if crate::key_crypto::is_encrypted(&bytes) {
        ...
        return crate::key_crypto::decrypt_key(&bytes, &pass).map_err(|e| e.to_string());
    }
...
fn verify_sigmanifest(
    ...
    ed25519_verify(pubkey, &message, &signature).map_err(|_| "SigVerifyFailed")
}
...
fn unhex32(s: &str) -> Result<[u8; 32], String> {
    ...
    bytes
        .try_into()
        .map_err(|_| "expected 32-byte (64-char) hex".into())
}
...
fn unhex16(s: &str) -> Result<[u8; 16], String> {
    ...
    bytes
        .try_into()
        .map_err(|_| "expected 16-byte (32-char) hex".into())
}
...
mod tests {
    ...
    fn sign_verify_round_trip() {
        ...
        verify_sigmanifest(bundle_bytes, &manifest, &pubkey).expect("round-trip must verify");
    }
