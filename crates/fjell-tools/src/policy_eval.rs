...
pub mod tests {
    ...
    const POLICY: &[Rule] = &[
        ...
        Rule {
            ...
            res: RC_TASK_CONTROL,
            kind: Kind::Allow,
            rights: TASK_MGMT | LEASE_RIGHTS,
        },
        Rule {
            req: INIT,
            res: RC_ENDPOINT,
            kind: Kind::Allow,
            rights: EP_RW | (1 << 8), /* MINT */
        },
        // service-manager
        Rule {
            req: SVC_MANAGER,
    ...
    fn evaluate(requester: u16, resource: u16, requested: u64) -> Verdict {
        ...
        for r in POLICY {
            ...
            if sm && resource == RC_ANY {
                continue;
            } // skip wildcard-resource deny for known resources
            if rm && sm {
    ...
    fn any_resource_denied_for_all_requesters() {
        for req in [INIT, SVC_MANAGER, AUDITD, STORAGED, 99u16] {
            assert_eq!(
                evaluate(req, RC_ANY, ALL_RIGHTS),
                Verdict::Denied,
                "requester {} should not get Any resource",
                req
            );
        }
    }
    ...
    fn init_endpoint_granted() {
        assert!(matches!(
            evaluate(INIT, RC_ENDPOINT, EP_RW),
            Verdict::Granted(_)
        ));
    }
    ...
    fn auditd_mmio_denied() {
        assert_eq!(
            evaluate(AUDITD, RC_MMIO_REGION, RIGHT_MMIO_MAP),
            Verdict::Denied
        );
    }
    ...
    fn cap_broker_self_query() {
        assert!(matches!(
            evaluate(CAP_BROKER, RC_ANY, ALL_RIGHTS),
            Verdict::Granted(_)
        ));
    }
    ...
    fn proxy_text_task_control_denied() {
        assert_eq!(
            evaluate(PROXY_TEXT, RC_TASK_CONTROL, TASK_MGMT),
            Verdict::Denied
        );
    }
