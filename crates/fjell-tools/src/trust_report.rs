...
pub fn cmd_trust_report(args: &[String]) -> ExitCode {
    ...
    match fs::write(out_path, &report) {
        ...
        Err(e) => {
            eprintln!("[trust-report] write error: {}", e);
...
fn section_3_measurement_chain() -> String {
    ...
    if prebuilt.exists() {
        let bins: Vec<_> = fs::read_dir(prebuilt)
            .ok()
            .into_iter()
            .flatten()
            .flatten()
...
fn section_4_catalog_binding() -> String {
    ...
    s.push_str(&format!(
        "  Schema digest   : (computed by fjell-semantic-toolkit)\n"
    ));
...
fn count_catalog_entries() -> usize {
    ...
    fs::read_to_string(path)
        ...
        .map(|s| {
            s.lines()
                .filter(|l| l.trim().starts_with("IntentEntry { tag:"))
                .count()
        })
        .unwrap_or(0)
}
...
mod tests {
    ...
    fn parse_count_extracts_number() {
        assert_eq!(
            parse_count("test result: ok. 487 passed; 0 failed", "passed"),
            Some(487)
        );
        assert_eq!(
            parse_count("test result: ok. 487 passed; 0 failed", "failed"),
            Some(0)
        );
