...
pub(crate) fn fe_zero() -> Fe {
    [0u64; 5]
}
pub(crate) fn fe_one() -> Fe {
    [1u64, 0, 0, 0, 0]
}
...
pub(crate) fn fe_from_bytes(b: &[u8; 32]) -> Fe {
    ...
    let mut buf = [0u64; 4];
    for i in 0..4 {
        buf[i] = u64::from_le_bytes(b[i * 8..i * 8 + 8].try_into().unwrap());
    }
...
pub(crate) fn fe_to_bytes(h: &Fe) -> [u8; 32] {
    ...
    let mut b = [0u8; 32];
    for i in 0..4 {
        b[i * 8..i * 8 + 8].copy_from_slice(&buf[i].to_le_bytes());
    }
    b
}
