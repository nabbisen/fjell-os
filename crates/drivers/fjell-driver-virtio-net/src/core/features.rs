...
impl VirtioFeatureFlags {
    ...
    pub fn without(self, bit: u64) -> Self {
        Self(self.0 & !bit)
    }
}
...
pub const DRIVER_ACCEPTED_FEATURES: u64 = VIRTIO_NET_F_MAC | VIRTIO_NET_F_STATUS;
...
mod feature_tests {
    ...
    fn negotiation_rejects_missing_mac() {
        ...
        assert_eq!(
            negotiate_features_checked(offered),
    ...
    fn negotiation_strips_unsupported_bits() {
        ...
        let offered =
            VirtioFeatureFlags(VIRTIO_NET_F_MAC | VIRTIO_NET_F_STATUS | VIRTIO_NET_F_MRG_RXBUF);
    ...
    fn modern_mode_detected_with_version_1() {
        const VIRTIO_F_VERSION_1: u64 = 1 << 32;
        let offered =
            VirtioFeatureFlags(VIRTIO_NET_F_MAC | VIRTIO_NET_F_STATUS | VIRTIO_F_VERSION_1);
        let (_, legacy) = negotiate_features_checked(offered).unwrap();
        assert!(!legacy);
    }
}
