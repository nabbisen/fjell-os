...
pub fn encode(
    ...
    for (i, fd) in schema.fields.iter().enumerate() {
        ...
        if fv != FieldValue::Absent {
            needed += fd.kind.wire_size();
        }
    }
    if out.len() < needed {
        return Err(SemanticError::BufferTooSmall);
    }
    ...
    for (i, fd) in schema.fields.iter().enumerate() {
        if fd.required {
            let fv = fields.get(i).copied().unwrap_or(FieldValue::Absent);
            if fv == FieldValue::Absent {
                return Err(SemanticError::RequiredFieldMissing);
            }
        }
    }
...
pub fn decode(bytes: &[u8]) -> Result<DecodedIntent, SemanticError> {
    if bytes.len() < 7 {
        return Err(SemanticError::TruncatedMagic);
    }
    if &bytes[..7] != MAGIC {
        return Err(SemanticError::BadMagic);
    }
    if bytes.len() < 17 {
        return Err(SemanticError::TruncatedBody);
    }
    ...
    for (i, fd) in schema.fields.iter().enumerate() {
        ...
        pos += 1;
        if present > 1 {
            return Err(SemanticError::InvalidPresentByte);
        }
        if present == 0 {
        ...
        fields[i] = match fd.kind {
            ...
            FieldKind::Bytes16 => {
                let mut b = [0u8; 16];
                b.copy_from_slice(&bytes[pos..pos + 16]);
                pos += 16;
                FieldValue::Bytes16(b)
            }
            FieldKind::Bytes32 => {
                let mut b = [0u8; 32];
                b.copy_from_slice(&bytes[pos..pos + 32]);
                pos += 32;
                FieldValue::Bytes32(b)
            }
        };
    ...
    if bytes[pos] != SENTINEL {
        return Err(SemanticError::MissingSentinel);
    }
    pos += 1;
    if pos != bytes.len() {
        return Err(SemanticError::TrailingBytes);
    }
