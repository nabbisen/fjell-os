...
| Every byte decoder a host fuzz crate can reach is fuzzed | RFC v0.6-003, RFC-0.32-001 | **DONE** (0.32; corrected 2026-09-25) — six decoders, all six fuzzed on CI for 300 s at the 0.32.0 cut, dispatch run `35089305545`. Decoders a host fuzz crate cannot reach are named in E-043's closure, not counted. *(This row said "six … run `34976532420`", and was wrong: that run fuzzed a different six, and there were seven decoders from RFC-0.32-002 until RFC-0.33-005 deleted `fjell-dtb-derive`; the target `dtb_validate` changed after that run and awaits a CI `fuzz-run`.)* *(This row read "Fuzz targets (≥ 4)" until 2026-09-15. Eight target files satisfied it while five had never compiled and one fuzzed anything; a count of targets was the defect.)* |
