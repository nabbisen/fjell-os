...
**Governed by:** [RFC-0.27-003](https://github.com/nabbisen/fjell-os/blob/main/rfcs/done/RFC-0.27-003-standards-mapping.md)
**Milestone:** 0.27
**Re-verified at:** every release cut, per
[the v0 release cycle](../releasing/v0-release-cycle.md) (R5).
...
have. See [RFC-0.27-003](https://github.com/nabbisen/fjell-os/blob/main/rfcs/done/RFC-0.27-003-standards-mapping.md)'s
...
| CRA-I-2b | (2)(b) Secure by default, incl. reset to original state | partial | TOFU trust-anchor provisioning behind an explicit `--allow-tofu-provision` flag (no silent default trust); reset-to-original-state and factory/hardware-anchored provisioning tiers are deferred to v1.1/v2+ | [v1-limitations.md](../releasing/v1-limitations.md); [RFC-v0.17-001](https://github.com/nabbisen/fjell-os/blob/main/rfcs/done/RFC-v0.17-001-trust-anchor-provisioning.md) |
| CRA-I-2c | (2)(c) Security updates, incl. automatic/opt-out/notification | partial | Signed, content-addressed bundle updates with an anti-rollback counter exist; there is no automatic-update, opt-out, or notification UX — updates are operator-driven | [RFC-v0.9-004](https://github.com/nabbisen/fjell-os/blob/main/rfcs/done/RFC-v0.9-004-bundle-builder-and-signed-service-package.md); [RFC-v0.11-003](https://github.com/nabbisen/fjell-os/blob/main/rfcs/done/RFC-v0.11-003-bundle-signing-pipeline-and-key-material-management.md); [RFC-v0.3-003](https://github.com/nabbisen/fjell-os/blob/main/rfcs/done/RFC-v0.3-003-anti-rollback-metadata-and-upgraded-local-confirmation-hardening.md) |
| CRA-I-2d | (2)(d) Protection from unauthorised access, incl. reporting | met | Capability system: every syscall accessing a resource requires a handle with matching kind and rights (`require_cap`); cap-broker enforces manifests with operator approval for grants | [capability-system.md](../architecture/capability-system.md); [threat-model-v1.md](../security/threat-model-v1.md) (T1, T2) |
| CRA-I-2e | (2)(e) Confidentiality of stored/transmitted data | partial | Signing-key material is encrypted at rest (RFC-v0.16-006). Bulk data-in-transit confidentiality is not production-ready: `fjell-sxt-crypto`, the only crate implementing it, states in its own doc-comment that it must not be used in production (unvetted AES with a documented cache-timing leak) and gates compilation behind an explicit acknowledgement feature; its consumer `secure-transportd` is currently a smoke-test stub | [RFC-v0.16-006](https://github.com/nabbisen/fjell-os/blob/main/rfcs/done/RFC-v0.16-006-key-handling-encryption-patch.md); [fjell-sxt-crypto/src/lib.rs](https://github.com/nabbisen/fjell-os/blob/main/crates/fjell-sxt-crypto/src/lib.rs); [v1-limitations.md](../releasing/v1-limitations.md) |
...
| CRA-I-2h | (2)(h) Availability of essential functions incl. after incident, DoS resilience | partial | Fleet partition FSM keeps a coordinator-less fleet visibly degraded rather than fabricating authority; no dedicated in-kernel resource-exhaustion or DoS mitigation instrument was found in this project's mechanism inventory | [threat-model-v1.md](../security/threat-model-v1.md) (T13); [RFC-v0.13-002](https://github.com/nabbisen/fjell-os/blob/main/rfcs/done/RFC-v0.13-002-fleet-split-reconnect-and-reconciliation.md) |
| CRA-I-2i | (2)(i) Minimise negative impact on other devices'/networks' availability | not-applicable | Fjell's network-facing services (`netd`, `virtio-net`, `secure-transportd`) are smoke-test stubs at v1 with no live network dataplane; nothing exists yet through which Fjell could impact another device's or network's availability | [v1-limitations.md](../releasing/v1-limitations.md) |
| CRA-I-2j | (2)(j) Limit attack surface incl. external interfaces | met | `syscall-surface` subcheck mechanically holds declared, dispatched, and committed-expectation syscall sets equal; the capability system bounds every external interface to an explicit grant | [syscall_surface.rs](https://github.com/nabbisen/fjell-os/blob/main/tools/fjell-consistency-check/src/syscall_surface.rs); [capability-system.md](../architecture/capability-system.md) |
| CRA-I-2k | (2)(k) Reduce incident impact via exploitation mitigation | met | `forbid(unsafe_code)` except at an audited, annotated boundary; unsafe-audit and MMIO-audit release gates mechanically enforce a checked justification comment at every unsafe/MMIO site | [release_rehearsal.rs](https://github.com/nabbisen/fjell-os/blob/main/crates/fjell-tools/src/release_rehearsal.rs) (Gates 2-3); [threat-model-v1.md](../security/threat-model-v1.md) (T4, T6) |
| CRA-I-2l | (2)(l) Security-related recording/monitoring, opt-out | partial | Kernel-managed audit ring records security-relevant events, drained via a capability-gated syscall; there is no user-facing opt-out because there is no end-user surface at the component level — audit is currently an operator/service concern | [ring.rs](https://github.com/nabbisen/fjell-os/blob/main/crates/fjell-kernel/src/audit/ring.rs); [threat-model-v1.md](../security/threat-model-v1.md) (T16) |
| CRA-I-2m | (2)(m) Secure, permanent removal of data and settings | not-met | No factory-reset or secure-erase user-facing mechanism was found. A narrower and more load-bearing gap is already recorded: even in-memory key erasure (`ZeroizeOnDrop`) is unverified | [v1-non-goals.md](../releasing/v1-non-goals.md) |
...
| ID | Requirement | Status | Mechanism | Evidence |
|----|-------------|--------|-----------|----------|
| CRA-II-1 | (1) Identify/document vulnerabilities and components, incl. SBOM | not-met | SBOM emission is BIZ-01 and unbuilt. Published vulnerabilities in components **are** now identified mechanically: every tracked `Cargo.lock` is checked against the RustSec database on every CI run, and the report states its surface — the two published crates carry no third-party dependencies, so it covers build and host tooling. That identifies vulnerabilities; it does not document components, which is what an SBOM is | [advisory-process.md](../security/advisory-process.md) (§7) |
| CRA-II-2 | (2) Address/remediate without delay, security updates separable | roadmap | The advisory process commits to acknowledgement within 7 days, a severity decision within 14 days of the report, and severity-tiered fix targets from 30 days (Critical) to the next regular release (Low) — targets, not guarantees, for a single-maintainer project; the bundle system can ship one service's update independently of others, but shipping a security update separately from a functionality update has never been exercised — no vulnerability has been reported against a released version (project is pre-1.0) | [SECURITY.md](https://github.com/nabbisen/fjell-os/blob/main/.github/SECURITY.md); [advisory-process.md](../security/advisory-process.md) |
| CRA-II-3 | (3) Effective and regular tests and reviews of security | met | A 12-gate release-rehearsal run at every cut; an errata register recording every found defect; a dedicated adversarial review pass against the threat model and non-goals | [release_rehearsal.rs](https://github.com/nabbisen/fjell-os/blob/main/crates/fjell-tools/src/release_rehearsal.rs); [adversarial-review-v0.16.md](../security/adversarial-review-v0.16.md); [ERRATA.md](https://github.com/nabbisen/fjell-os/blob/main/rfcs/ERRATA.md) |
| CRA-II-4 | (4) Share/publicly disclose fixed vulnerabilities | roadmap | A disclosure process and an advisory register exist, the register checked by Gate 12 while empty — no vulnerability has been reported. Neither has been exercised: nothing has been reported and fixed pre-1.0, and no rehearsal has been run (E-051's named survivor) | [SECURITY.md](https://github.com/nabbisen/fjell-os/blob/main/.github/SECURITY.md); [advisory-process.md](../security/advisory-process.md); [advisories/](../security/advisories/README.md) |
| CRA-II-5 | (5) Coordinated vulnerability disclosure policy | met | Private GitHub security-advisory intake as the one channel, an explicit no-public-issue instruction, acknowledgement within 7 days and a severity decision within 14 — stated identically in SECURITY.md and the process, and held equal by a Gate 12 subcheck. *Before 0.32.0 this verdict was stronger than its evidence: the commitment was "a small number of days", and the release checklist published a second address that could not receive mail (E-051)* | [SECURITY.md](https://github.com/nabbisen/fjell-os/blob/main/.github/SECURITY.md); [advisory-process.md](../security/advisory-process.md) |
| CRA-II-6 | (6) Facilitate sharing of vulnerability info, contact address | met | SECURITY.md publishes the reporting channel as the single point of contact. *"Single" has been true only since 0.32.0, when the release checklist's unreachable `security@<domain>` was removed (E-051)* | [SECURITY.md](https://github.com/nabbisen/fjell-os/blob/main/.github/SECURITY.md); [advisory-process.md](../security/advisory-process.md) (§1) |
| CRA-II-7 | (7) Mechanisms to securely distribute updates, automatic where applicable | partial | Signed, content-addressed bundle distribution with anti-rollback exists; automatic distribution/application is not implemented — fleet updates are deliberately operator-driven | [RFC-v0.9-004](https://github.com/nabbisen/fjell-os/blob/main/rfcs/done/RFC-v0.9-004-bundle-builder-and-signed-service-package.md); [v1-non-goals.md](../releasing/v1-non-goals.md) (N20) |
| CRA-II-8 | (8) Disseminate without delay, free of charge, with advisory messages | roadmap | No update has ever been disseminated in response to a security issue (pre-1.0). The advisory record that would accompany one is now documented; the free-of-charge commitment is not | [advisory-process.md](../security/advisory-process.md) (§5) |
...
- **RFC-v0.7.3-002's own specified deliverable docs do not exist.** That RFC
  ...
  Filed as **E-028** (`rfcs/ERRATA.md`), ACCEPTED, `unscheduled` — not
