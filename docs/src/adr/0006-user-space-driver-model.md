# ADR-0006 — User-Space Driver Model
...
MMIO and DMA boundaries are the highest-risk kernel boundaries.
At v0.1.x both are unenforced (see RFC 030 audit). v0.2 closes them
via RFC 035 (MmioRegion) and RFC 036 (DmaRegion).

## Deferred Work

- MmioRegion capability ABI (v0.2: RFC 035).
- DmaRegion capability + zeroize/quarantine (v0.2: RFC 036).
- IOMMU integration (v0.3).
- DTB-driven device discovery (v0.3).
- Multi-page scatter-gather DMA (when a device needs it).

## Related RFCs

- RFC 005 (MMIO RAM Exclusion), RFC 007 (Per-Task DMA Allocator)
- RFC 016 (MMIO Region Capability stub), RFC 017 (DMA Region Capability stub)
- RFC 035, RFC 036 (v0.2 closure)
- RFC 030 (MMIO/DMA Boundary Audit, v0.1.3)
