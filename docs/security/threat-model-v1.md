...
**Adversary:** C-SUPPLY  
**Defence:** RFC-v0.10-003 (reproducible build gate), corrected by RFC-0.30-001. Two *genuinely independent* builds — a scoped `cargo clean` precedes each one, so the second cannot reuse the first's incremental output — compared by SHA-256 digest (hardened from FNV-1a in RFC-v0.16-005, H-04). Runs in CI on every push (`ci-repro-check`) — *corrected 2026-09-12, E-041: that job has never succeeded; apt's `rust-src` cannot `build-std`, so it fails before building. The check is real and demonstrated locally; it is not yet run anywhere but a developer's machine.* Covers the kernel ELF and all 29 service prebuilts (30 artefacts); verified bit-for-bit identical across two independent runs on 2026-09-09. This is **same-machine** reproducibility only. The toolchain that produced each artefact is now recorded with it (RFC-0.30-003: `rustc -vV`'s release/commit-hash/host/LLVM, on the repro baseline, every evidence provenance file and the trust report), but the channel is unpinned within `1.91.x` and nothing compares two machines' digests against each other (E-037's two surviving instances), so cross-machine reproducibility is not tested here and this line does not claim it. The separate, unrelated `--skip-build` tier (test-all, CI-cheap) checks only that the *committed* prebuilts have not gone stale against their recorded baseline; it does not build anything and is not this defence.  
**Residual:** Malicious toolchain that produces identical output for different inputs; cross-machine reproducibility, unproven — the toolchain is recorded but unpinned, and no two machines' digests are ever compared (E-037). *Corrected at the 0.30.0 cut under exit criterion 8: this row said "the toolchain is not yet recorded" for two days after RFC-0.30-003 recorded it.*
